...
function getNextFriday(date) {
  ...
  const daysCountToAdd =
    currentDay >= 5 ? 7 - (currentDay - 5) : 5 - currentDay;
  firstDate.setDate(firstDate.getDate() + daysCountToAdd);
  return firstDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diffDays = (endDate - startDate) / oneDay + 1;
  return diffDays;
}
...
function formatDate(date) {
  ...
  const time = new Date(date).toLocaleTimeString('en-US', { timeZone: 'UTC' });
 ...
 */
// function getWorkSchedule({ start }) {
//   const stringDate = new Date(start);
//   const options = { year: 'numeric', month: 'numeric', day: 'numeric' };
//   console.log(
//     stringDate.toLocaleDateString('ru', options),
//     '--------------------'
//   );
// }
const getFormattedString = (argDate) => {
  const yyyy = argDate.getFullYear();
  let mm = argDate.getMonth() + 1; // Months start at 0!
  let dd = argDate.getDate();
...
function getWorkSchedule({ start, end }, countWorkDays, countOffDays) {
  ...
  let currentWorkDays = countWorkDays;
  let currentOffDays = countOffDays;

  while (currentDate <= endDate) {
    if (currentWorkDays > 0) {
      currentWorkDays -= 1;
      arr.push(getFormattedString(currentDate));
      currentDate.setDate(currentDate.getDate() + 1);
    } else if (currentOffDays > 0) {
      currentOffDays -= 1;
      currentDate.setDate(currentDate.getDate() + 1);
    } else {
      currentWorkDays = countWorkDays;
      currentOffDays = countOffDays;
    }
  }
  return arr;
}
